...
<script>
	...
	wa4 - (wa + wa2 + wa3);
	wa5 - (wa + wa2 + wa3 + wa4);
	wa6 - (wa + wa2 + wa3 + wa4 + wa5);

	if (sum >= 1) {
		passdata1 = Math.round(sum) / (10 - wa);
	} else {
		passdata1 = 0;
	}
	if (sum2 >= 1) {
		passdata2 = Math.round(sum2) / (10 - wa2);
	} else {
		passdata2 = 0;
	}
	if (sum3 >= 1) {
		passdata3 = Math.round(sum3) / (10 - wa3);
	} else {
		passdata3 = 0;
	}
	if (sum4 >= 1) {
		passdata4 = Math.round(sum4) / (10 - wa4);
	} else {
		passdata4 = 0;
	}
	if (sum5 >= 1) {
		passdata5 = Math.round(sum5) / (10 - wa5);
	} else {
		passdata5 = 0;
	}
	if (sum6 >= 1) {
		passdata6 = Math.round(sum6) / (10 - wa6);
	} else {
		passdata6 = 0;
	}
</script>
	...
	<div id="center">
		<form name="nForm">
			...
			<tr>
				...
				<div class="kokuban-t2"><span class="title-t2">あなたはアナウンサータイプ！</span>アナウンサーのようにいつも<ruby>笑顔<rt>えがお</rt></ruby>
					を<ruby>絶<rt>た</rt></ruby>やさずおだやかです。どんなときでも、<ruby>冷静<rt>れいせい</rt></ruby>に<ruby>自分<rt>じぶん</rt></ruby>をコントロールすることができ、<ruby>周<rt>まわ</rt></ruby>りにも<ruby>気<rt>き</rt></ruby>を<ruby>配<rt>くば</rt></ruby>れます。また、
					<ruby>自分<rt>じぶん</rt></ruby>の<ruby>気持<rt>きも</rt></ruby>ちを<ruby>言葉<rt>ことば</rt></ruby>にして<ruby>伝<rt>つた</rt></ruby>えることがじょうずです。そんな<ruby>強<rt>つよ</rt></ruby>みを<ruby>活<rt>い</rt></ruby>かして、<ruby>様々<rt>さまざま</rt></ruby>なことにチャレンジしたり、
					<ruby>周<rt>まわ</rt></ruby>りの<ruby>人<rt>ひと</rt></ruby>を<ruby>盛<rt>も</rt></ruby>り<ruby>上<rt>あ</rt></ruby>げたり、<ruby>自分<rt>じぶん</rt></ruby>にも<ruby>人<rt>ひと</rt></ruby>にもパワーを<ruby>与<rt>あた</rt></ruby>えましょう！
					<!--メモの表示・非表示-->
