...
<div id="center">
	...
	try{
		...
		foreach ($user_detail_disability as $user_detail_disability) {
		}
		...
		if($details['cnt']==1){
			$detail = $dbh ->prepare('UPDATE user_detail SET grade=?, gender=?, disability=?, comment=? WHERE id =?');
			$detail->execute(array($user_detail_grade, $user_detail_gender, $user_detail_disability, $user_detail_comment, $uid));
			$details = $detail ->fetch();
		} else if($details['cnt']==0){
			$detail = $dbh ->prepare('INSERT user_detail SET id=?, grade=?, gender=?, disability=?, comment=?');
			$detail->execute(array($uid, $user_detail_grade, $user_detail_gender, $user_detail_disability, $user_detail_comment));
			$details = $detail ->fetch();
		}
